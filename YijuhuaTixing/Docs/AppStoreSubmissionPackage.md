...
- 上一公开版本：`1.0`，已在中国香港、中国澳门分发。
- 本次提交版本：`1.1`，用于第二版 App Store 上架和首次软著申请材料。
- 软著版本号：`V1.1`。
- 开发完成日期：`2026-06-24`。
- 上架状态：`1.1` 当前为准备提交状态，提交 App Store Connect 前需新建 iOS App 版本 `1.1` 并选择 Build `4`。
- 发审策略：隐藏未接真实后端的个人信息/账号入口，保留本地提醒、会员中心、协议链接、反馈和设置能力。

### 1.1 更新摘要

- 新增提醒地址输入和外部地图跳转，支持 Apple 地图、高德、百度、腾讯、Google 等已安装地图 App。
- 优化新建/详情页时间滚轮触控宽度，减少上下滑动误触。
- 默认早上/晚上时间支持 30 分钟步进，并统一用于解析器、通知动作和快捷建议。
- 优化待办与历史分组，覆盖当天、本周、本月、本季度、半年、本年及更远时间段。
- 扩展重复提醒解析，支持每天、每小时、每 N 小时、每周、每双周、每月、工作日。
- 优化自然语言解析，包含标签关键词、节假日之后、点后分钟与标题数字边界等真实场景。
...
| 季度会员 | `com.yijuhua.pro.quarterly` | 3 months | `¥15/季度` | `季度会员` |
| 年度会员 | `com.yijuhua.pro.annual` | 1 year | `¥58/年` | `年度会员` |
...
- 账号：`1.1` 隐藏个人信息/账号入口，正式构建不提供登录、注册或模拟账号能力。
...
5. 在确认页或详情页填写地址后，可点击地图按钮跳转外部地图 App；地址仅保存在本机，本 App 不申请定位权限、不在 App 内展示地图、不上传地址。

1.1 版本不提供账号登录入口，提醒数据保存在本机。
